...
const AutogeneratedFormConfigurator: React.FC = () => {
    ...
    const { dsCode, dataElements, sectionCodes, deInSectionCodes, categoryComboCodes } = useMemo(() => {
        const selectedDS = _.find(dataSets, { id: filters.dataSetId });
        if (!selectedDS)
            return { dsCode: "", sectionCodes: [], categoryComboCodes: [], dataElements: [], deInSectionCodes: [] };
        ...
        const categoryComboCodes = _(selectedDS.dataSetElements)
            .map(dataSetElement => dataSetElement.categoryCombo?.code)
            .compact()
            .uniq()
            .value();

        return { dsCode: selectedDS.code, sectionCodes, categoryComboCodes, dataElements, deInSectionCodes };
    ...
    useEffect(() => {
        compositionRoot.dataStoreConfig.getConstants().then(constants => setConstants(constants));
    }, [compositionRoot.dataStoreConfig]);
    ...
    return (
        <Container>
            ...
            <ButtonGroup>
                ...
                <Button onClick={() => setValue("{}")} color="default" variant="contained">
                    {i18n.t("Clear")}
                </Button>
            </ButtonGroup>
        </Container>
