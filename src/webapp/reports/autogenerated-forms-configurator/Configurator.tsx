...
const AutogeneratedFormConfigurator: React.FC = () => {
    ...
    const { code, dataElements, sections } = useMemo(() => {
        const selectedDS = _.find(dataSets, { id: filters.dataSetId });
        if (!selectedDS) return { code: "", sections: [], dataElements: [] };

        setValue(!value ? "{}" : value);

        const dataElements = selectedDS.dataSetElements.map(dataSetElement => dataSetElement.dataElement);

        return { code: selectedDS.code, sections: selectedDS.sections, dataElements };
