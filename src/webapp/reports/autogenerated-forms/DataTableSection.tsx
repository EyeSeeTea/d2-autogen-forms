...
import { evaluateTotalRule, verifyConditionByDataValueType } from "./hooks/useApplyRules";
...
const DataTableSection: React.FC<DataTableProps> = React.memo(props => {
    ...
    const isSectionVisible = React.useMemo(() => {
        if (toggle.type === "dataElementExternal" && !isSectionOpen) return false;
        ...
        if (sectionTotalRules.length !== 0 && !isSectionVisibleFromTotals) return false;
        ...
        return true;
    }, [toggleMultiple, sectionTotalRules.length, isSectionVisibleFromTotals, toggle, isSectionOpen, dataFormInfo]);
