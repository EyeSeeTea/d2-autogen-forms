...
const AutogeneratedForm: React.FC = () => {
    const [dataFormInfo, isLoading, rules] = useDataFormInfo();
    ...
    return (
        <div ref={dataFormInfo.initForm}>
            ...
            {rules.length > 0 && (
                <AlertContainer>
                    {rules.map(rule => {
                        return <AlertRule key={rule.validationRuleId} visible message={rule.message || ""} />;
                    })}
                </AlertContainer>
            )}
...
function useDataFormInfo(): [Maybe<DataFormInfo>, boolean, ValidationResult[]] {
    ...
    const saveDataValue = useCallback<DataFormInfo["data"]["save"]>(
        ...
        [compositionRoot, dataValues, dataForm, snackbar, key]
    );
    ...
    return [dataFormInfo, isLoading, rules];
}
