import { Dhis2ConfigRepository } from "./data/common/Dhis2ConfigRepository";
import { Dhis2OrgUnitsRepository } from "./data/common/Dhis2OrgUnitsRepository";
import { GetConfig } from "./domain/common/usecases/GetConfig";
import { GetOrgUnitsUseCase } from "./domain/common/usecases/GetOrgUnitsUseCase";
import { D2Api } from "./types/d2-api";
import { getReportType } from "./webapp/utils/reportType";
import { Dhis2DataFormRepository } from "./data/common/Dhis2DataFormRepository";
import { SaveDataFormValueUseCase } from "./domain/common/usecases/SaveDataFormValueUseCase";
import { GetDataFormUseCase } from "./domain/common/usecases/GetDataFormUseCase";
import { GetDataFormValuesUseCase } from "./domain/common/usecases/GetDataFormValuesUseCase";
import { Dhis2DataValueRepository } from "./data/common/Dhis2DataValueRepository";
import { ApplyToAllUseCase } from "./domain/common/usecases/ApplyToAllUseCase";
import { SaveGridWithTotalsValueUseCase } from "./domain/common/usecases/SaveGridWithTotalsValueUseCase";
import { AutogenConfigDataStoreRepository } from "./data/autogenerated-forms-configurator/AutogenConfigDataStoreRepository";
import { Dhis2CustomFormRepository } from "./data/autogenerated-forms-configurator/Dhis2CustomFormRepository";
import { GetDataSetsUseCase } from "./domain/autogenerated-forms-configurator/usecases/GetDataSetsUseCase";
import { GetFormConfigUseCase } from "./domain/autogenerated-forms-configurator/usecases/GetFormConfigUseCase";
import { GetInstallStateUseCase } from "./domain/autogenerated-forms-configurator/usecases/GetInstallStateUseCase";
import { InstallCustomFormUseCase } from "./domain/autogenerated-forms-configurator/usecases/InstallCustomFormUseCase";
import { SaveFormConfigUseCase } from "./domain/autogenerated-forms-configurator/usecases/SaveFormConfigUseCase";
import { ImportConstantTranslationsUseCase } from "./domain/common/usecases/ImportConstantTranslationsUseCase";
import { CreateConstantUseCase } from "./domain/common/usecases/CreateConstantUseCase";
import { ConstantD2Repository } from "./data/ConstantD2Repository";
import { ImportConstantFsRepository } from "./data/ImportConstantFsRepository";
import { DataElementD2Repository } from "./data/DataElementD2Repository";
import { ExportDataElementConfigFsRepository } from "./data/ExportDataElementConfigFsRepository";
import { ExportConstantFsRepository } from "./data/ExportConstantFsRepository";
import { ValidateDatasetRulesUseCase } from "./domain/autogenerated-forms-configurator/usecases/ValidateDatasetRulesUseCase";
import { RuleD2Repository } from "./data/common/RuleD2Repository";
import { Dhis2DataSetRepository } from "./data/autogenerated-forms-configurator/Dhis2DataSetRepository";
import { DeleteFormConfigUseCase } from "./domain/autogenerated-forms-configurator/usecases/DeleteFormConfigUseCase";
import { DeleteDataFormValuesUseCase } from "./domain/common/usecases/DeleteDataFormValuesUseCase";
import { GetDataSetUseCase } from "./domain/autogenerated-forms-configurator/usecases/GetDataSetWithElementsUseCase";
import { SaveDataSetUseCase } from "./domain/autogenerated-forms-configurator/usecases/CreateDefaultSectionUseCase";

export function getCompositionRoot(api: D2Api) {
    const configRepository = new Dhis2ConfigRepository(api, getReportType());
    const orgUnitsRepository = new Dhis2OrgUnitsRepository(api);
    const dataFormRepository = new Dhis2DataFormRepository(api);
    const dataValueRepository = new Dhis2DataValueRepository(api);
    const dataStoreConfigRepository = new AutogenConfigDataStoreRepository(api);
    const dataSetRepository = new Dhis2DataSetRepository(api);
    const customFormRepository = new Dhis2CustomFormRepository(api);
    const constantRepository = new ConstantD2Repository(api);
    const importConstantRepository = new ImportConstantFsRepository();
    const dataElementRepository = new DataElementD2Repository(api);
    const exportDataElementConfigRepository = new ExportDataElementConfigFsRepository();
    const exportConstantRepository = new ExportConstantFsRepository();
    const ruleRepository = new RuleD2Repository(api);

    return {
        dataForms: getExecute({
            get: new GetDataFormUseCase(dataFormRepository, orgUnitsRepository),
            getValues: new GetDataFormValuesUseCase(dataValueRepository),
            saveValue: new SaveDataFormValueUseCase(dataValueRepository),
            deleteValues: new DeleteDataFormValuesUseCase(dataValueRepository),
            applyToAll: new ApplyToAllUseCase(dataValueRepository),
            saveWithTotals: new SaveGridWithTotalsValueUseCase(dataValueRepository),
        }),
        dataStoreConfig: getExecute({
            getFormConfig: new GetFormConfigUseCase(dataStoreConfigRepository),
            saveFormConfig: new SaveFormConfigUseCase(dataStoreConfigRepository),
            deleteFormConfig: new DeleteFormConfigUseCase(dataStoreConfigRepository),
            getDataSets: new GetDataSetsUseCase(dataSetRepository),
        }),
        customForm: getExecute({
            getInstallState: new GetInstallStateUseCase(customFormRepository),
            install: new InstallCustomFormUseCase(customFormRepository, dataSetRepository),
        }),
        orgUnits: getExecute({
            get: new GetOrgUnitsUseCase(orgUnitsRepository),
        }),
        config: getExecute({
            get: new GetConfig(configRepository),
        }),
        constants: getExecute({
            import: new ImportConstantTranslationsUseCase(
                constantRepository,
                importConstantRepository,
                dataElementRepository,
                exportDataElementConfigRepository,
                exportConstantRepository
            ),
            create: new CreateConstantUseCase(constantRepository),
        }),
        dataSet: getExecute({
            validate: new ValidateDatasetRulesUseCase(ruleRepository),
            getById: new GetDataSetUseCase(dataSetRepository),
            save: new SaveDataSetUseCase(dataSetRepository),
        }),
    };
}

export type CompositionRoot = ReturnType<typeof getCompositionRoot>;

function getExecute<UseCases extends Record<Key, UseCase>, Key extends keyof UseCases>(
    useCases: UseCases
): { [K in Key]: UseCases[K]["execute"] } {
    const keys = Object.keys(useCases) as Key[];
    const initialOutput = {} as { [K in Key]: UseCases[K]["execute"] };

    return keys.reduce((output, key) => {
        const useCase = useCases[key];
        const execute = useCase.execute.bind(useCase) as UseCases[typeof key]["execute"];
        output[key] = execute;
        return output;
    }, initialOutput);
}

export interface UseCase {
    execute: Function;
}
