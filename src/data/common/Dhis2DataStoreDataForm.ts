...
import { Namespaces, dataStoreNamespace, formerDataStoreNamespace } from "./clients/storage/Namespaces";
...
export class Dhis2DataStoreDataForm {
    ...
    static async build(api: D2Api, dataSetCode?: string): Promise<Dhis2DataStoreDataForm> {
        ...
        let dataStore = api.dataStore(dataStoreNamespace);
        if (!dataSetCode) dataStore = api.dataStore(formerDataStoreNamespace);
        const storeValue = await dataStore
            .get<object>(dataSetCode ? dataSetCode : Namespaces.AUTOGENERATED_FORMS)
            .getData();
        if (!storeValue)
